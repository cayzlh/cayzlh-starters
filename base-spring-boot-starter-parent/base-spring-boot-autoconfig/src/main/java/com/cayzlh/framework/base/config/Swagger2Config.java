...
public class Swagger2Config {
    ...
    public String[] getBasePackages() {
        ...
        if (StringUtils.isBlank(basePackage)) {
            log.error("com.cayzlh.base.swagger.base-package (扫描基本包)不可为空，请检查配置文件");
            throw new SwaggerConfigException("Swagger basePackage不能为空");
        }
    ...
    private List<Parameter> getParameters() {
        // 获取自定义参数配置
        List<SwaggerProperties.ParameterConfig> parameterConfig = swaggerProperties
                .getParameterConfig();
    ...
    @Bean
    public IndexController indexController(BaseProperties baseProperties) {
        return new IndexController(baseProperties);
    }
